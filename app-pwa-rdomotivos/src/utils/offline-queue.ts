...
/** Optional callback invoked after successful flush (e.g. to invalidate queries) */
...
// Auto-flush with debounce when back online
  ...
  window.addEventListener('online', () => {
    ...
    flushTimeout = setTimeout(async () => {
      ...
      if (ok > 0) {
        ...
        useNotificationStore.getState().addToast('success', `${ok} apontamento(s) sincronizado(s)`);
      }
    }, 1500);
  });
}
