...
import {
  ...
  Info, ExpandMore, ExpandLess, Delete, Sync, Warning, DeleteSweep,
  Settings,
    ...
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
        <Typography variant="caption" fontWeight={600}>
          {expired ? 'Sessao expirada' : `${minutes}m ${seconds}s`}
        </Typography>
        <Chip label={expired ? 'Expirado' : 'Ativo'} size="small" color={expired ? 'error' : 'success'}
          sx={{ height: 20, fontSize: '0.6rem' }} />
      </Box>
      <LinearProgress variant="determinate" value={progress} color={color} sx={{ height: 6, borderRadius: 3 }} />
    </Box>
  );
}
...
function OfflineQueueSection() {
  ...
  const refresh = useCallback(async () => {
    setPending(await getPendingCount()); setFailed(await getFailedCount());
      ...
      <CardContent sx={{ pb: '12px !important' }}>
        ...
        {failed > 0 && (
          <Button size="small" startIcon={<Sync />}
            onClick={async () => { setSyncing(true); await retryFailed(); await refresh(); setSyncing(false); }}
            disabled={syncing} sx={{ mt: 1 }}>
            {syncing ? 'Sincronizando...' : 'Reenviar'}
          </Button>
        )}
          ...
          <Box sx={{ mt: 1 }}>
            {items.map((item) => (
              <Box key={item.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 0.5, borderTop: 1, borderColor: 'divider' }}>
                ...
                <IconButton size="small" onClick={async () => { await remove(item.id); await refresh(); }} sx={{ p: 0.25 }}>
...
export function ConfigPage() {
  ...
  const dbColors: Record<DatabaseEnv, string> = { PROD: '#EF4444', TESTE: '#3B82F6', TREINA: '#22C55E' };

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pb: 2, maxWidth: 480, mx: 'auto' }}>
      {/* Page Title */}
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pt: 1, px: 0.5 }}>
        <Settings sx={{ fontSize: 24, color: 'primary.main' }} />
        <Typography variant="h6" fontWeight={800}>Configuracoes</Typography>
      </Box>
        ...
        <CardContent sx={{ pb: '12px !important' }}>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            {(['PROD', 'TESTE', 'TREINA'] as DatabaseEnv[]).map((db) => (
              <Button key={db} size="small" variant={database === db ? 'contained' : 'outlined'}
                onClick={() => handleDbChange(db)}
                sx={{
                  flex: 1, fontSize: '0.75rem', fontWeight: 700,
                  bgcolor: database === db ? dbColors[db] : 'transparent', borderColor: dbColors[db],
                  color: database === db ? 'white' : dbColors[db],
                  '&:hover': { bgcolor: database === db ? dbColors[db] : alpha(dbColors[db], 0.08), borderColor: dbColors[db] },
                }}>
                {db}
              </Button>
            ))}
      ...
      <Card variant="outlined">
        <CardContent sx={{ pb: '12px !important' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
            ...
            <Typography variant="subtitle2" fontWeight={700}>Aparencia</Typography>
          </Box>
          <FormControlLabel
      ...
      <Button variant="outlined" color="warning" startIcon={<DeleteSweep />}
        onClick={() => setClearDialogOpen(true)} fullWidth sx={{ fontWeight: 700 }}>
      ...
      <Dialog open={clearDialogOpen} onClose={() => setClearDialogOpen(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            Isso vai remover todos os dados salvos localmente, limpar o cache offline,
            desregistrar o service worker e exigir um novo login.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setClearDialogOpen(false)}>Cancelar</Button>
          <Button onClick={async () => { setClearDialogOpen(false); await uninstallPwa(); }}
            color="error" variant="contained">Limpar Tudo</Button>
        </DialogActions>
      </Dialog>
      <Button variant="outlined" color="error" startIcon={<Logout />}
        onClick={handleLogout} fullWidth sx={{ fontWeight: 700 }}>
        Sair
      </Button>
    </Box>
  );
