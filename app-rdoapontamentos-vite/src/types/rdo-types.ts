...
export interface RdoResumoDiario {
  data: string;
  totalRdos: number;
  totalMinutos: number;
  totalFuncionarios: number;
...
export interface RdoResumoDiarioResponse {
  data: RdoResumoDiario[];
  meta: {
    total: number;
    page: number;
    limit: number;
...
export interface RdoSearchResult {
  CODRDO: number;
  CODPARC: number | null;
  DTREF: string | null;
  nomeparc: string | null;
...
export interface RdoMotivoFormData {
  DESCRICAO: string;
  SIGLA: string;
  ATIVO: 'S' | 'N';
  PRODUTIVO: 'S' | 'N';
...
export interface MotivosListParams {
  page?: number;
  limit?: number;
  ativo?: 'S' | 'N';
  orderBy?: string;
...
/* ── Detalhes por periodo (GET /rdo/detalhes) ── */
