...
export function FuncionarioCombobox({
  ...
  onChange,
  label = 'Funcionario',
  placeholder = 'Digite nome, codigo ou departamento...',
  disabled = false,
  required = false,
  size = 'small',
  fullWidth = true,
  ...
  const handleInputChange = useCallback(
    (_: unknown, newInput: string, reason: string) => {
      ...
      // Don't search when user selects an option (reason=reset) or clears (reason=clear)
      ...
      debounceRef.current = setTimeout(async () => {
        setLoading(true);
        try {
          const results = await searchFuncionarios(newInput);
          setOptions(results);
        } catch {
          setOptions([]);
        } finally {
