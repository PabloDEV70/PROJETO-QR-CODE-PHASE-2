...
function invalidateChamados(qc: ReturnType<typeof useQueryClient>) {
  ...
  qc.invalidateQueries({ queryKey: ['chamados', 'chat-list'] });
}
...
export function useDeleteOcorrencia() {
  ...
  return useMutation({
    mutationFn: ({ nuchamado, sequencia }: { nuchamado: number; sequencia: number }) =>
      deleteOcorrencia(nuchamado, sequencia),
    onSuccess: (_data, { nuchamado }) => {
      qc.invalidateQueries({ queryKey: ['chamados', 'ocorrencias', nuchamado] });
      qc.invalidateQueries({ queryKey: ['chamados', 'chat-list'] });
      addToast('success', 'Tratativa removida');
    },
    onError: (err: Error) => {
      addToast('error', `Erro ao remover tratativa: ${err.message}`);
    },
  });
}
...
export function useUpdateChamadoStatus() {
  ...
  return useMutation({
    ...
    onError: (err: unknown, _vars, context) => {
      ...
      const detail = axErr?.response?.data?.message || axErr?.response?.data?.error || axErr?.message || 'Erro desconhecido';
      console.error('[useUpdateChamadoStatus] FAILED:', detail, err);
      addToast('error', `Erro ao alterar status: ${detail}`);
    },
    onSettled: (_data, _err, { nuchamado }) => {
      invalidateChamados(qc);
      qc.invalidateQueries({ queryKey: ['chamados', 'detail', nuchamado] });
      qc.invalidateQueries({ queryKey: ['chamados', 'ocorrencias', nuchamado] });
    },
