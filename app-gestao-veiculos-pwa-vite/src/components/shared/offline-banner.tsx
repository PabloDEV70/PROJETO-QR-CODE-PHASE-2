    ...
    <Box sx={{
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 0.75 }}>
        ...
        {failedCount > 0 && isOnline && (
          <Button size="small" startIcon={<Sync sx={{ fontSize: 14 }} />} onClick={handleRetry} disabled={syncing}
      ...
      <Collapse in={expanded}>
        <Box sx={{ px: 2, pb: 1 }}>
          {items.map((item) => (
            <Box key={item.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 0.5, borderTop: 1, borderColor: alpha('#92400E', 0.15) }}>
              <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: item.status === 'failed' ? '#EF4444' : item.status === 'syncing' ? '#3B82F6' : '#F59E0B', flexShrink: 0 }} />
          ...
          {items.length === 0 && (
            <Typography sx={{ fontSize: '0.7rem', color: alpha('#92400E', 0.6), py: 0.5 }}>Nenhum item na fila</Typography>
          )}
        </Box>
      </Collapse>
    </Box>
