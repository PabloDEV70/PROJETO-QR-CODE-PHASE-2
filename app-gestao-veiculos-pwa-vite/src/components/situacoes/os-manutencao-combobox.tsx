...
export function OsManutencaoCombobox({ value: _value, onChange, disabled }: OsManutencaoComboboxProps) {
  ...
  return (
    <Autocomplete
      ...
      onChange={(_, opt) => { setSelected(opt); onChange(opt?.NUOS ?? ''); }}
      options={options}
      loading={loading}
      disabled={disabled}
      fullWidth
      ...
      renderInput={(params) => <TextField {...params} label="OS Manutencao (NUOS)" placeholder="Buscar por nº OS ou placa..." />}
    />
  );
}
