...
export function DatabaseSelector() {
  ...
  return (
    <Stack spacing={1} alignItems="center">
      ...
      <ToggleButtonGroup value={database} exclusive onChange={(_, val: DatabaseEnv | null) => { if (val) setDatabase(val); }} size="small" sx={{ width: '100%' }}>
        {DB_OPTIONS.map((opt) => (
          <ToggleButton key={opt.value} value={opt.value} sx={{ flex: 1, fontSize: '0.75rem', py: 0.5, '&.Mui-selected': { bgcolor: `${opt.color}14`, color: opt.color, borderColor: `${opt.color}40`, '&:hover': { bgcolor: `${opt.color}20` } } }}>
            {opt.label}
          </ToggleButton>
        ))}
      </ToggleButtonGroup>
