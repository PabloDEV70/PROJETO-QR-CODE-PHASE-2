...
interface TotpRecoveryFormProps {
  totpToken: string;
  onRecover: (totpToken: string, code: string) => Promise<LoginResponse>;
  onSuccess: (data: LoginResponse) => void;
  onBack: () => void;
}

export function TotpRecoveryForm({ totpToken, onRecover, onSuccess, onBack }: TotpRecoveryFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await onRecover(totpToken, code);
      onSuccess(data);
    } catch (err) {
