...
function parseLoginError(error: unknown, apiBaseUrl?: string): ParsedError {
  ...
  if (!axiosErr.response || code === 'ERR_NETWORK') {
    return {
      ...
      hint: apiBaseUrl ? 'Verifique se a API esta rodando em ' + apiBaseUrl : 'Verifique sua conexao.',
      severity: 'error',
      icon: <WifiOff />,
      code,
    };
...
export function LoginError({ error, onClose, apiBaseUrl }: LoginErrorProps) {
  const parsed = parseLoginError(error, apiBaseUrl);
