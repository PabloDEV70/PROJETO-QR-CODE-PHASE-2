...
import { getProdBenchmarkStatus, buildCategoryBreakdown } from '@/utils/wrench-time-categories';

/**
 * Computes per-collaborator wrench time using wtCategoria from the DB.
 * When rankingMap is provided, uses backend produtividadePercent as the
 * authoritative productivity number (accounts for almoco, banheiro, fumar).
 * Source of truth: /rdo/analytics/ranking endpoint.
 */
export function computeColabWrenchTime(
  ...
  return Array.from(colabMap.entries()).map(([codparc, info]) => {
    ...
    // Use backend produtividadePercent from ranking (single source of truth)
