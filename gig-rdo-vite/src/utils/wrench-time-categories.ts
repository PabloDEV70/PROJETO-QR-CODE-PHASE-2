...
// Re-export so existing importers of computeColabWrenchTime still work
export { computeColabWrenchTime } from '@/utils/wrench-time-colab';

/** Presentation metadata per DB WTCATEGORIA key (labels, colors, tips) */
...
/** Benchmark for backend produtividadePercent (range ~70-110%) */
...
/** Shared helper: build breakdowns from a category-minutes map */
...
/**
 * Compute wrench time metrics using backend-provided deductions (wtSummary).
 * All deduction values (almoco excess, banheiro excess) come from the API wtSummary.
 * Frontend only groups motivos into categories for chart rendering — no business logic.
 * See also: computeColabWrenchTime in wrench-time-colab.ts for per-collaborator breakdown.
 */
export function computeWrenchTimeMetrics(
  ...
  // Build category map — almoco/banheiro use only excess (from backend)
