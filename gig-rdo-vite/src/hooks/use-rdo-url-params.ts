...
import type { RdoListParams, RdoDetalhesParams } from '@/types/rdo-types';
...
export function useRdoUrlParams() {
  ...
  const listParams = useMemo<RdoListParams>(() => {
    const p: RdoListParams = { page, limit, dataInicio, dataFim };
  ...
  const filterParams = useMemo<Omit<RdoDetalhesParams, 'page' | 'limit'>>(() => {
    const p: Omit<RdoDetalhesParams, 'page' | 'limit'> = { dataInicio, dataFim };
    if (codparc) p.codparc = codparc;
