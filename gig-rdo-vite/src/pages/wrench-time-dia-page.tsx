...
import { useRdoResumo, useRdoDetalhes } from '@/hooks/use-rdo';
...
export function WrenchTimeDiaPage() {
  ...
  const updateParams = useCallback(
    (updates: Record<string, string | null>) => {
      ...
      if (newDate && newDate !== dtref) {
        ...
        const s = qs.toString();
        navigate(`/rdo/wrench-time/dia/${newDate}${s ? `?${s}` : ''}`);
        return;
      }
  ...
  const rdoDetalhes = useRdoDetalhes({ ...filterParams, page: 1, limit: 100 });
